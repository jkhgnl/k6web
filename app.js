...
(function () {
  ...
  async function checkUpdate() {
    ...
    const currentEl = $("updCurrent");
    const latestEl = $("updLatest");
    const publishedEl = $("updPublished");
    const bodyBox = $("updBodyBox");

    btn.disabled = true;
    applyBtn.disabled = true;
    status.textContent = "正在检查更新...";
    status.className = "hint";
    currentEl.textContent = K5WEB_VERSION || "0.1.0";
    ...
    try {
      ...
      if (IS_GITHUB_PAGES) {
        // GitHub Pages 模式：直接从 Gitee API 获取最新 Release
        const checkUrl = `https://gitee.com/api/v5/repos/jkhgnl/k6-web/releases/latest?_=${t}`;
        const resp = await fetchWithTimeout(checkUrl, {}, 15000);
        const raw = await resp.json();
        // 将 Gitee API 格式转换为内部统一格式
        const tagName = (raw.tag_name || "").replace(/^v/, "");
        const asset = (raw.assets || []).find(a => a.name === "k5web-release.zip");
        data = {
          ok: true,
          current: K5WEB_VERSION,
          latest: tagName || K5WEB_VERSION,
        ...
        // 本地服务器模式：通过服务器代理检查更新
        const resp = await fetchWithTimeout(`/update/check?t=${t}`, {}, 15000);
      ...
      publishedEl.textContent = data.published_at ? `（发布于 ${data.published_at}）` : "";
      if (data.body) {
        bodyBox.textContent = data.body;
        bodyBox.style.display = "block";
      }

      if (data.latest === K5WEB_VERSION) {
        status.textContent = "当前已是最新版本。";
        status.className = "hint ok";
        log(`更新检查完成：当前 ${data.current}，远程 ${data.latest}（已是最新）`);
      } else {
        // GitHub Pages 版本：提示用户手动下载更新
        const hint = IS_GITHUB_PAGES
          ? `检测到新版本 ${data.latest}，请点击下方按钮前往 Gitee Release 下载更新包。`
          : `检测到新版本 ${data.latest}，可点击「立即更新」。`;
        status.textContent = hint;
        status.className = "hint ok";
        if (!IS_GITHUB_PAGES && data.has_asset) {
          applyBtn.disabled = false;
        }
        log(`检测到新版本 ${data.latest}`);
      }
    } catch (err) {
      status.textContent = "检查更新失败：" + err.message;
      status.className = "hint err";
      log("检查更新失败：" + err.message, "err");
    } finally {
      btn.disabled = false;
    }
  }
  ...
  async function applyUpdate() {
    // GitHub Pages 模式：跳转到 Gitee Release 页面供用户手动下载
    if (IS_GITHUB_PAGES) {
      window.open("https://gitee.com/jkhgnl/k6-web/releases", "_blank");
      return;
    }

    if (!updateInfo || !updateInfo.has_asset) {
      log("没有可用的更新包", "err");
      return;
    }
    if (!confirm(`确定要更新到 ${updateInfo.latest} 吗？\n更新会覆盖本地所有发布文件并重启服务器。\n请确保当前没有正在进行的写频/刷写操作。`)) {
      return;
    }
    ...
    status.textContent = "正在下载并应用更新，请稍候...";
    status.className = "hint";
    progress.style.display = "block";
    bar.style.width = "30%";
    ...
    try {
      const t = Date.now();
      const resp = await fetchWithTimeout(`/update/apply?t=${t}`, { method: "POST" }, 60000);
      bar.style.width = "80%";
      const data = await resp.json();
      ...
      log(data.message);
      setTimeout(() => {
        status.textContent = "服务器正在重启，约 5-10 秒后将自动刷新页面...";
  ...
  $("updCurrent").textContent = K5WEB_VERSION || "0.1.0";
